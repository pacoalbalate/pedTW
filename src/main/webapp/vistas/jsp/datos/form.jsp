		...
		<div class="table-responsive">
			<div class="table-wrapper">
				...
				<div>
					...
					<form:form
						action="${pageContext.request.contextPath}/centro/${datosfecha.centro.id}/datos/save"
						modelAttribute="datosfecha" method="POST">
						...
						<div class="form-group row">
							...
							<div class="col-sm-6 row">
								<spring:bind path="tipoprueba">
									<form:radiobuttons items="${tipopruebas}" path="tipoprueba"
						...
						<div class="form-group row">
							<label class="col-sm-2 col-form-label">Pruebas
								realizadas:</label>
							<div class="col-sm-6">
								<spring:bind path="totalpruebas">
									<form:input type="number" path="totalpruebas"
										class="form-control ${status.error ? 'alert-danger' : ''}" />
				...
				<c:if test="${datosfecha.id >0}">
					...
					<div class="container">
						<button type="button"
							class="btn ${(datosperfil.id>0) ? 'btn-success ' : 'btn-info '}"
							data-toggle="collapse" data-target="#divfiltros">
							${(datosperfil.id>0) ? 'Editar ' : 'Nuevo '} Perfil</button>
						<label class="col-sm-10 col-form-label text-right">Total
							Positivos: ${sumapositivos} </label>

						<div id="divfiltros"
							class="collapse ${((datosperfil.id>0) || (danger != null)) ? 'show ' : ''}">

							<div>


								<form:form
									action="${pageContext.request.contextPath}/centro/${datosfecha.centro.id}/datos/${datosfecha.id}/perfil/save"
									modelAttribute="datosperfil" method="POST">

									<div class="form-group row">
										<label class="col-sm-2 col-form-label">Positivos:</label>
										<div class="col-sm-6">
											<spring:bind path="totalpositivos">
												<form:input type="number" path="totalpositivos"
													class="form-control ${status.error ? 'alert-danger' : ''}" />
												<form:errors path="totalpositivos"
										...
										<c:choose>
											<c:when test="${d.pregunta.tipopregunta.opcion =='Texto'}">
												<div class="form-group row">
													...
													<div class="col-sm-6">
														<input type="text" class="form-control" value="${d.dato}"
															name="${d.pregunta.denominacion}"
															id="${d.pregunta.denominacion}" />
													</div>
												</div>
											</c:when>
												...
												<div class="form-group row">
													...
													<div class="col-sm-6">
														<input type="number" class="form-control"
															value="${d.dato}" name="${d.pregunta.denominacion}"
															id="${d.pregunta.denominacion}" />
													</div>
												</div>
											</c:when>
											<c:when test="${d.pregunta.tipopregunta.opcion =='Fecha'}">
												<div class="form-group row">
													<label class="col-sm-2 col-form-label">${d.pregunta.denominacion}:</label>
													<div class="col-sm-6">
														<input type="date" class="form-control" value="${d.dato}"
															name="${d.pregunta.denominacion}"
															id="${d.pregunta.denominacion}" />
													</div>
												</div>
														...
														<div class="col-sm-6 row">
															<select class="form-control"
																...
																<c:forEach var="opc" items="${opcionesdatos}">
																	<c:if
																		test="${d.pregunta.tipopregunta.opcionestabla == opc.tipo}">
																		<option value="${opc.opcion}"
																			${d.dato==opc.opcion ? 'selected="selected"' : ''}>${opc.opcion}</option>
																	</c:if>
																</c:forEach>
															</select>
														</div>
				...
				<c:if test="${datosfecha.getDatosperfiles().isEmpty()}">
					<div class="alert alert-info my-4">No tiene perfiles de datos
						asociados...</div>
				</c:if>
					...
					<table class="table table-striped table-hover table-responsive">
						...
						<tbody>
							<c:forEach var="reg" items="${datosfecha.datosperfiles}">
								<c:url var="linkVer"
									value="${pageContext.request.contextPath}/centro/${reg.id}/"></c:url>
								...
								<tr>
									<c:if test="${preguntas.size()>8}">
										<td><a href="${linkSave}" class="edit"><i
											...
											data-toggle="modal"><i class="material-icons"
												data-toggle="tooltip" title="Borrar">&#xE872;</i></a></td>
									</c:if>
									...
									<c:forEach var="d" items="${reg.datos}">
										...
										<c:if test="${d.pregunta.tipopregunta.opcion =='Fecha'}">
											<td><fmt:parseDate value="${d.dato}"
													pattern="yyyy-MM-dd" var="date" /> <fmt:formatDate
													type="date" value="${date}" pattern="yyyy-MM-dd" /></td>
										</c:if>
										<c:if test="${d.pregunta.tipopregunta.opcionestabla !=''}">
