...
<form:form action="${pageContext.request.contextPath}/admin/region/save" modelAttribute="region" method="POST">
...
<form:input type="number" path="habitantes" class="form-control ${status.error ? 'alert-danger' : ''}"/>
...
<div class="alert alert-info my-4">No tiene centros asociados...</div>
</c:if>

<c:forEach var="reg" items="${region.centros}">
		<c:url var="linkVer" value="${pageContext.request.contextPath}/region/${reg.id}/"></c:url>
							...
							<tr>
								...
								<td><a href="${linkSave}" class="edit"><i
									...
									<a href="${linkDes}" class="delete" data-id="${reg.id}">
										...
										data-toggle="tooltip" title="Desasociar">drive_file_move</i></a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
