		...
		<div class="table-responsive">
			<div class="table-wrapper">
				...
				<div>
					...
					<form:form
						action="${pageContext.request.contextPath}/usuario/${regId}/admin/save"
								...
								<spring:bind path="clave">
									<div class="input-group">
										<form:input type="password" path="clave"
											class="form-control pwd ${status.error ? 'alert-danger' : ''}" />
								...
								<sec:authorize access="hasRole('ROLE_GESTOR')">
									<select name="tipo_rol" id="tipo_rol" class="form-control">
										<c:forEach var="cto" items="${tiporoles}">
											<option value="${cto.opcionestabla}" Label="${cto.opcion}"
												${cto.opcionestabla==tipo_rol ? 'selected' : ''} />
										</c:forEach>
									</select>
								</sec:authorize>
								<sec:authorize access="hasRole('ROLE_REGION')">
									<input type="hidden" name="tipo_rol" id="tipo_rol"
										class="form-control" value="ROLE_CENTRO" />
								</sec:authorize>
							</div>
						</div>
						...
						<c:if test="${ctoreg_rol>0}">
							<div class="form-group row">
								<label class="col-sm-2 col-form-label">Acceso permitido
									a:</label>
								<div class="col-sm-6">
									<c:if test="${ctoreg_rol >0}">
									...
									<c:if test="${ctoreg_rol<1}">
										<label class="form-control">Pendiente. Sin
											autorización asociada...</label>
									</c:if>
								</div>
							</div>
						</c:if>
				...
				<c:if
					test="${(tipo_rol=='ROLE_CENTRO') or (tipo_rol=='ROLE_REGION')}">
					<form:form action="asoc" method="POST">
						...
						<table class="table table-striped table-hover table-responsive">
							<tbody>
								...
								<tr>
									<td>Seleccione ${(tipo_rol=='ROLE_CENTRO') ? 'el centro al' : 'la región a la'}
										que desea permitir el acceso:</td>
									<td></td>
									<td><c:if test="${(tipo_rol=='ROLE_CENTRO')}">
											<select name="centro" id="centro" class="form-control">
												<c:forEach var="cto" items="${centros}">
													<option value="${cto.id}"
														Label="Centro: ${cto.id} ${cto.denominacion}"
														${cto.id==ctoreg_rol ? 'selected' : ''} />
												</c:forEach>
											</select>
										</c:if> <c:if test="${(tipo_rol=='ROLE_REGION')}">
											<select name="centro" id="centro" class="form-control">
												<c:forEach var="cto" items="${regiones}">
													<option value="${cto.id}"
														Label="Region: ${cto.id} ${cto.denominacion}"
														${cto.id==ctoreg_rol ? 'selected' : ''} />
												</c:forEach>
											</select>
										</c:if></td>
									<td></td>
									<td><input type="submit" value="Asociar"
